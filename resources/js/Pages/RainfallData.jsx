...
export default function RainfallData({ auth, rainfalls, filters }) {
    ...
    const handlePerPageChange = (e) => {
        ...
        router.get(route('rainfall.data'), { 
            per_page: newValue,
                    ...
                    <div className="p-4 bg-gray-50 border-b border-gray-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center space-x-3 mb-2 lg:mb-0">
                            <div className="p-2 bg-blue-100 rounded-lg text-blue-600 shadow-sm">
                                <FontAwesomeIcon icon={faCloudShowersHeavy} size="lg" />
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-gray-800 leading-tight">Data Riwayat</h3>
                                <p className="text-xs text-gray-500">Laporan sensor realtime</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
                            ...
                            <div className="flex items-center bg-white border border-gray-300 rounded-lg overflow-hidden w-full sm:w-auto shadow-sm">
                                ...
                                />
                                <span className="bg-gray-100 border-x border-gray-300 px-2 py-2 text-xs text-gray-500">-</span>
                                <input 
                                    type="date" 
                                    value={endDate}
                    ...
                    <div className="overflow-x-auto w-full flex-1">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-white sticky top-0 z-10 shadow-sm">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap bg-gray-50">Waktu (WITA)</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap bg-gray-50">Curah Hujan</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap bg-gray-50">Tip Count</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap bg-gray-50">Event ID</th>
                                </tr>
                            </thead>
                                        ...
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                    item.rainfall > 0 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-500'
                                                }`}>
                                                    {item.rainfall} mm
                                                </span>
                                            </td>
                    ...
                    <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                            <div className="text-xs text-gray-500 text-center sm:text-left">
                                Show <span className='font-bold'>{rainfalls.from}-{rainfalls.to}</span> of {rainfalls.total}
                            </div>
                        ...
                        <div className="flex space-x-1 overflow-x-auto max-w-full pb-1 sm:pb-0">
                            {rainfalls.links.map((link, index) => {
                                ...
                                const baseClasses = "px-2 sm:px-3 py-1 text-xs font-medium rounded-md border transition-colors duration-150 whitespace-nowrap";

                                if (isLinkDisabled) {
                                    return <span key={index} dangerouslySetInnerHTML={{ __html: link.label }} className={`${baseClasses} bg-gray-100 text-gray-300 border-gray-200 cursor-not-allowed`} />;
                                }
                                return (
                                    <Link 
                                        key={index} 
                                        href={link.url} 
                                        data={{ per_page: perPage, start_date: startDate, end_date: endDate }}
